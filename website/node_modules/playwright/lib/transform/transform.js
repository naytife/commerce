...
function resolveHook(filename, specifier, isESM) {
  if (specifier.startsWith('node:') || builtins.has(specifier)) return;
  if (!shouldTransform(filename)) return;
  if (isRelativeSpecifier(specifier)) return (0, _util.resolveImportSpecifierExtension)(_path.default.resolve(_path.default.dirname(filename), specifier), false, isESM);
  ...
  for (const tsconfig of tsconfigs) {
    ...
    for (const {
      ...
      for (const value of values) {
        ...
        const existing = (0, _util.resolveImportSpecifierExtension)(candidate, true, isESM);
        if (existing) {
  ...
  if (_path.default.isAbsolute(specifier)) {
    ...
    return (0, _util.resolveImportSpecifierExtension)(specifier, false, isESM);
  }
}
