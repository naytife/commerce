<script lang="ts">
	...
	$:productQuery = createQuery<Product>({
		...
		queryFn: () => api(authFetch).getProductById(data.productId),
	})
</script>
					...
					<Card.Content>
						<div class="grid gap-6">
							<div class="grid gap-3">
								...
								<Input id="name" type="text" class="w-full" value={$productQuery.data.title} />
							</div>
							<div class="grid gap-3">
								<Label for="description">Description</Label>
								<Textarea
									id="description"
									value={$productQuery.data.description}
									class="min-h-32"
								/>
							</div>
						</div>
