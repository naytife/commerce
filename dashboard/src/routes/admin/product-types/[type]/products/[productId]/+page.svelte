<script lang="ts">
	...
	const productQuery = createQuery<ExtendedProduct>({
		...
		queryFn: async () => {
			return api(authFetch).getProductById(data.productId);
		},
	});
	...
	const productImagesQuery = createQuery<ProductImage[]>({
		...
		queryFn: async () => {
			return api(authFetch).getProductImages(data.productId);
		},
	...
	const productTypeQuery = createQuery<ProductType>({
		...
		queryFn: async() => {
			...
			return api(authFetch).getProductTypeById(productTypeId);
		},
	...
	const typeAttributesQuery = createQuery<ProductTypeAttribute[]>({
		...
		queryFn: async () => {
			if (!productTypeId) throw new Error('Product type ID not available');
			return api(authFetch).getProductTypeAttributes(productTypeId);
		},
	});
	...
	async function updateProduct() {
		try {
			const updatedProduct = await api(authFetch).updateProduct(data.productId, {
				title,
				description,
				attributes,
			...
			await queryClient.invalidateQueries({ queryKey: ['product', data.productId] });
		} catch (error) {
	...
	async function handleImageUpload(event: Event) {
		...
		try {
			...
			try {
				await api(authFetch).addProductImage(productId, {
				...
				// Invalidate and refetch the query
				await queryClient.invalidateQueries({ queryKey: ['product-images', data.productId] });
			} catch (error) {
	...
	async function deleteImage(image: ProductImage) {
		...
		try {
			...
			const deleted = await api(authFetch).deleteProductImage(productId, image.id);
	...
	async function setPrimaryImage(image: ProductImage) {
		...
		try {
			await api(authFetch).setProductPrimaryImage(productId, image.id);
			...
			if ($productImagesQuery.data) {
				...
				if (imageIndex > 0) {
					...
					queryClient.setQueryData(['product-images', data.productId], reorderedImages);
				}
			}
		} catch (error) {
	...
	{#if $productQuery.data}
		<div class="mx-auto grid max-w-[59rem] flex-1 auto-rows-max gap-4">
			<div class="flex items-center gap-4">
				<Button variant="outline" size="icon" class="h-7 w-7" href="/admin/products">
