<script lang="ts">
	...
	const productTypeQuery = createQuery({
		queryKey: ['product-type', data.typeId],
		queryFn: () => api().getProductTypeById(data.typeId)
	});
	...
	const attributesQuery = createQuery<Attribute[]>({
		queryKey: ['product-type-attributes', data.typeId],
		queryFn: () => api().getProductTypeAttributes(data.typeId)
	});
	...
	async function handleAttributeSaved() {
		await queryClient.invalidateQueries({ queryKey: ['product-type-attributes', data.typeId] });
	}
	...
	async function handleUpdateProductType() {
		...
		try {
			await api().updateProductType(data.typeId, {
				title,
			...
			await queryClient.invalidateQueries({ queryKey: ['product-type', data.typeId] });
		} catch (error) {
	...
	async function handleDeleteProductType() {
		...
		try {
			await api().deleteProductType(data.typeId);
